...
cmd='HCopy -C mfcc13.cfg -S output\wav2fea.scp';
dos(cmd);
...
cmd='outMacro.exe P D 3 1 MFCC_E 13 > output\template.hmm';
dos(cmd);
...
fid=fopen('output\mxup.scp', 'w'); fprintf(fid, 'MU 2 {*.state[2-4].mix}'); fclose(fid);
...
% calculating Confusion matrix on Training data 
...
% calculating Confusion matrix on Testing data 
